...
import {FSU} from "../src/FSU.sol";

contract FSUTest is Test {
    FSU public token;
    address supplyOwnerAddress = makeAddr("MainChainWalletUser"); // 0xE4960346854Daa3Ef2c0444DC29341cEC2d245b3
    ...
    function test_transferRevertInsufficientBalance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("ERC20InsufficientBalance(address,uint256,uint256)", randomWalletAddress, 0, 100)
        );
    ...
    function test_transferFromRevertInsufficientAllowanceFor0x0() public {
        vm.prank(supplyOwnerAddress);
        vm.expectRevert(
            abi.encodeWithSignature("ERC20InsufficientAllowance(address,uint256,uint256)", supplyOwnerAddress, 0, 100)
        );
    ...
    function test_transferFromRevertInsufficientAllowance() public {
        ...
        vm.prank(randomWalletAddress);
        vm.expectRevert(
            abi.encodeWithSignature("ERC20InsufficientAllowance(address,uint256,uint256)", randomWalletAddress, 30, 50)
        );
